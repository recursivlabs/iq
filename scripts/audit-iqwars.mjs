...
const remindersUnsubscribePath = path.join(root, 'src/app/api/reminders/unsubscribe/route.ts');
...
async function sourceAudit() {
  ...
  const remindersUnsubscribe = source(remindersUnsubscribePath);
  ...
  assert(existsSync(remindersUnsubscribePath), 'Reminder unsubscribe route exists.');
  ...
  assert(researchSection.includes('https://www.testingstandards.net/open-access-files.html') && researchSection.includes('https://arxiv.org/abs/2108.08604') && researchSection.includes('https://doi.org/10.1037/0033-295X.97.3.404') && researchSection.includes('https://doi.org/10.1073/pnas.0801268105') && researchSection.includes('https://doi.org/10.1037/a0028228') && researchSection.includes('https://doi.org/10.1177/1529100616661983') && researchSection.includes('https://arxiv.org/abs/2302.04238'), 'Research page cites testing standards, IRT, matrix reasoning, cognitive training, and AI reasoning benchmark sources.');
  ...
  assert(app.includes('reasoning-game ranking for players, groups, and geographies') && app.includes('among active players') && app.includes('strongest active-player daily reasoning scores') && app.includes('becomes less noisy as more daily signals accumulate') && app.includes('90 plus accuracy times 52') && app.includes('speed bonus up to 8 points') && app.includes('100, 250, 500, and 1000 completed answers') && app.includes('not clinical IQ diagnoses, educational/admission decisions, employment signals, or proof of innate intelligence') && app.includes('not a clinical IQ test, admission test, employment screen, high-IQ society qualifier, or supervised psychometric assessment'), 'About page explains scoring/confidence while keeping conservative high-stakes boundaries.');
  ...
  assert(app.includes('function RoomRecordStrip') && app.includes('Room record chase') && app.includes('Today resets; old official scores stay on this room link.'), 'Friend room rankings surface all-time highscores as a primary room summary above the daily board.');
  ...
  assert(app.includes('command-panel sidebar-nav') && app.includes('{navStatus}') && app.includes('command-scroll') && app.includes('role="navigation"'), 'Navigation renders as a left sidebar drawer with account-state context.');
  assert(app.includes('overscroll-behavior: contain') && app.includes('.command-scroll') && app.includes('overflow: visible') && app.includes('max-height: none') && criticalCss.includes('overscroll-behavior: contain') && criticalCss.includes('max-height: none'), 'Command sidebar uses one intuitive drawer scroll instead of nested scrolling regions.');
  ...
  assert(visualAudit.includes('auditAgentVisibilityDefaults') && visualAudit.includes('keeps seeded benchmark profiles hidden by default') && visualAudit.includes('shows seeded benchmark profiles only after the setting is enabled'), 'Visual audit covers seeded benchmark visibility defaults and opt-in ranking mode.');
  ...
  assert(app.includes('function BrandAvatar') && app.includes('avatarModel') && app.includes('profile-identity') && app.includes('BrandAvatar seed={`${entry.playerId}:${entry.username || entry.displayName}`') && profiles.includes('Mina Vale') && leaderboard.includes('Mina Vale') && !leaderboard.includes(['Agent', 'Euclid'].join(' ')), 'Players and seeded benchmark profiles render stable brand-character default avatars with human-looking benchmark names.');
  ...
  assert(remindersLib.includes('MAX_REMINDERS') && remindersLib.includes('normalizeReminder') && remindersLib.includes('validEmail') && remindersLib.includes('new Map') && remindersLib.includes('reminderUnsubscribeToken') && remindersLib.includes('disabledAt'), 'Reminder store normalizes, dedupes, bounds records, and stores unsubscribe/disabled state before reads and writes.');
  ...
  assert(remindersSend.includes('buildReminderDigest') && remindersSend.includes('currentStreakDays') && remindersSend.includes('Next validity goal') && remindersSend.includes('Evidence:') && remindersSend.includes('Room record'), 'Reminder cron sends a short streak, score/evidence, validity-goal, and room-record digest instead of a generic nudge.');
  ...
  assert(remindersSend.includes('/api/reminders/unsubscribe?t=') && remindersSend.includes('reply STOP') && remindersSend.includes('!reminder.disabledAt') && remindersUnsubscribe.includes('IQ WARS reminders are off.'), 'Reminder emails include one-click unsubscribe and disabled reminders are skipped.');
  ...
  assert(reminderProof.includes('expectedFromDomain') && reminderProof.includes('emailHeaderDomain(from) === expectedFromDomain') && reminderProof.includes('redactEmailHeader(from)'), 'Reminder proof verifies the actual sent email uses the configured verified sender domain without leaking addresses.');
  assert(reminderProof.includes('Score: 137 IQ') && reminderProof.includes('Evidence: 12 answers') && reminderProof.includes('Next validity goal: 100 answers') && reminderProof.includes('Streak: 1 day') && reminderProof.includes('Best: 137 IQ') && reminderProof.includes('\\/api\\/reminders\\/unsubscribe\\?t=') && reminderProof.includes('Or reply STOP'), 'Reminder proof verifies score evidence, validity goal, streak, best, room-record, play-link, and unsubscribe copy in the delivered email.');
  ...
  assert(checkout.includes('function safeCheckoutUrl') && checkout.includes('register|login|sign-in|sign-up') && !checkout.includes('fallback: true'), 'Checkout API rejects generic Recursiv registration/login URLs instead of falling back to a dead-end payment link.');
  ...
  assert(checkoutProof.includes('maliciousReturnUrl') && checkoutProof.includes("checkoutSummary.host !== 'evil.example'") && checkoutProof.includes('generic account registration URL') && checkoutProof.includes('checkout.stripe.com') && checkoutProof.includes('buy.stripe.com'), 'Checkout proof verifies unsafe return URLs are not reflected and checkout does not return Recursiv registration.');
...
async function liveAudit() {
  ...
  const publicPages = [
    ...
    ['/u/mina_vale', ['IQ WARS'], 'Live public profile route renders the shell for a benchmark profile slug.'],
  ];
