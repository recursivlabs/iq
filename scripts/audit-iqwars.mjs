...
async function sourceAudit() {
  ...
  assert(recursivConfig.includes('RECURSIV_AUTH_CACHE_MS') && recursivConfig.includes('__iqwarsRecursivProjectAuthPromise') && recursivConfig.includes("status.error === 'rate_limit_exceeded'"), 'Recursiv project auth verifier caches successful readiness and shares in-flight checks so health probes do not rate-limit launch readiness.');
